...
for class_name in class_names:
    ...
    class_folders_val.append(os.path.join(validation_folder_path, class_name))
...
for idx,class_folder in enumerate(class_folders):
	...
	for file in class_files[:int(len(class_files)*0.7)]:
		copyfile(file, os.path.join(class_folders_train[idx],file.split('/')[-1]))
	for file in class_files[int(len(class_files)*0.7):]:
		print file
		print(os.path.join(class_folders_val[idx],file.split('/')[-1]))
		copyfile(file, os.path.join(class_folders_val[idx],file.split('/')[-1]))
